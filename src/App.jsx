...
const DOOHBiddingSystem = ({ currentView = 'standard' }) => {
  // 將 Hook 入面所有嘅 State 同 Function 全部解構出嚟 (一個都無少)
  const {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans pb-20 relative pt-0">
      <SEO title="DOOH Adv Platform - 自助廣告投放系統" />
      ...
      <main className="max-w-5xl mx-auto p-3 md:p-6 space-y-4 md:space-y-8 mt-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
          ...
          {/* 時間選擇器 (傳入 corporateSOV) */}
          <TimeSlotSelector 
            HOURS={HOURS} 
            previewDate={previewDate} 
            selectedScreens={selectedScreens} 
            occupiedSlots={occupiedSlots} 
            corporateSOV={corporateSOV} 
                  ...
                  <div className="pt-4 border-t flex flex-col gap-3">
                      ...
                      <div className="flex gap-3 mt-2">
                          <button 
                              ...
                              className="flex-1 bg-slate-100 text-slate-600 py-3 rounded-lg font-bold hover:bg-slate-200"
                          >
                              取消
                          </button>
                          <button 
          ...
          <div className="bg-white rounded-xl max-w-md w-full p-6 text-center">
            {transactionStep === 'summary' && pendingTransaction ? (
                <>
                    ...
                    <p className="mb-4">類型: {pendingTransaction.type === 'buyout' ? '鎖定時段 (即扣款)' : '一般競價 (預授權)'}</p>
