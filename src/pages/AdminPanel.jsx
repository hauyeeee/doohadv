...
import { 
  ...
  Edit, MapPin, Image as ImageIcon, Layers, FileText, Map, Copy, Gavel, Flag 
} from 'lucide-react';
import { 
...
const AdminPanel = () => {
  ...
  const handleBulkAction = async (act) => { if(selectedOrderIds.size===0)return; if(!confirm('確認操作？'))return; const b=writeBatch(db); selectedOrderIds.forEach(id=>{if(act==='cancel') b.update(doc(db,"orders",id),{status:'cancelled'})}); await b.commit(); alert("已完成"); setSelectedOrderIds(new Set()); };
  const handleAddRule = async () => { if(!newRule.date) return alert("請輸入日期"); let hours = []; const inputStr = newRule.hoursStr.trim(); if (!inputStr || inputStr.toLowerCase() === 'all') { hours = Array.from({length: 24}, (_, i) => i); } else { if (inputStr.includes('-')) { const [start, end] = inputStr.split('-').map(n => parseInt(n)); if (!isNaN(start) && !isNaN(end) && start <= end) { for (let i = start; i <= end; i++) if (i >= 0 && i <= 23) hours.push(i); } } else { hours = inputStr.split(',').map(h => parseInt(h.trim())).filter(h => !isNaN(h) && h >= 0 && h <= 23); } } if (hours.length === 0) return alert("❌ 時段格式錯誤"); try { await addDoc(collection(db, "special_rules"), { screenId: newRule.screenId, date: newRule.date, hours: hours, type: newRule.action, value: newRule.action === 'price_override' ? parseFloat(newRule.overridePrice) : null, note: newRule.note, createdAt: new Date() }); alert("✅ 規則已建立"); setNewRule({ ...newRule, hoursStr: '', overridePrice: '', note: '' }); } catch (e) { console.error(e); alert("❌ 建立失敗"); } };
  const handleDeleteRule = async (id) => { if(confirm("刪除規則？")) await deleteDoc(doc(db, "special_rules", id)); };
  const handleScreenChange = (fid, f, v) => setEditingScreens(p => ({ ...p, [fid]: { ...p[fid], [f]: v } }));
  const saveScreenSimple = async (s) => { const d = editingScreens[s.firestoreId]; if(d) { if(d.basePrice) d.basePrice = parseFloat(d.basePrice); await updateDoc(doc(db, "screens", s.firestoreId), d); alert("已儲存"); setEditingScreens(p=>{const n={...p};delete n[s.firestoreId];return n;}); } };
  const handleMarkAsScheduled = async (orderId) => { if (!confirm("確認已將影片編排至播放系統？")) return; try { await updateDoc(doc(db, "orders", orderId), { isScheduled: true, scheduledAt: new Date(), scheduledBy: user.email }); alert("✅ 狀態更新：準備播放"); if (selectedSlotGroup) { const updatedGroup = selectedSlotGroup.map(s => s.orderId === orderId ? { ...s, isScheduled: true, displayStatus: 'scheduled' } : s); setSelectedSlotGroup(updatedGroup); } } catch (e) { alert("更新失敗"); } };
 ...
 // 🔥🔥🔥 自動結算競價 (防彈版) - 修復 createdAt 導致的崩潰 🔥🔥🔥
  ...
  const toggleScreenActive = async (s) => { if(confirm("切換上架狀態？")) await updateDoc(doc(db, "screens", s.firestoreId), { isActive: !s.isActive }); };
