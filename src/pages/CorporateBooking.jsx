...
import { Map, MapPin, Building2, Train, Target, FileText, CheckCircle, ChevronRight, ChevronLeft, AlertTriangle, Users, CalendarRange, Clock, Download, BarChart3 } from 'lucide-react';

// --- V2 Mock Data (加入人流、受眾標籤) ---
const MOCK_SCREENS = [
  { id: 'S001', name: '旺角西洋菜南街巨屏', district: 'Mong Kok', mtr: 'Tsuen Wan Line', basePrice: 50, bannedCategories: ['alcohol', 'gambling'], footfall: 150000, audience: ['Gen Z', 'Shoppers'] },
  { id: 'S002', name: '尖沙咀廣東道大電視', district: 'Tsim Sha Tsui', mtr: 'Tsuen Wan Line', basePrice: 80, bannedCategories: [], footfall: 200000, audience: ['Tourists', 'High-Income'] },
  { id: 'S003', name: '銅鑼灣 SOGO 對出', district: 'Causeway Bay', mtr: 'Island Line', basePrice: 100, bannedCategories: [], footfall: 250000, audience: ['Shoppers', 'White-collar'] },
  { id: 'S004', name: '觀塘工業區大連排', district: 'Kwun Tong', mtr: 'Kwun Tong Line', basePrice: 40, bannedCategories: ['finance'], footfall: 120000, audience: ['White-collar', 'Students'] },
  { id: 'S005', name: '中環蘭桂坊入口', district: 'Central', mtr: 'Island Line', basePrice: 120, bannedCategories: ['kids'], footfall: 80000, audience: ['Expats', 'Nightlife', 'High-Income'] },
];
...
const DAYPARTING_OPTIONS = [
  ...
  { id: 'rush_hour', name: '黃金通勤 (08:00-10:00, 17:00-20:00)', hours: 5, multiplier: 1.5 }, // 黃金時段單價貴啲
...
const CorporateBooking = () => {
  ...
  // --- Screen & Strategy State ---
  const [regionMode, setRegionMode] = useState('district');
  const [selectedScreens, setSelectedScreens] = useState(new Set(MOCK_SCREENS.map(s => s.id)));
  ...
  const activeScreens = useMemo(() => {
    return MOCK_SCREENS.filter(s => selectedScreens.has(s.id) && (!industry || !s.bannedCategories.includes(industry)));
  }, [selectedScreens, industry]);
  ...
  const metrics = useMemo(() => {
    ...
    activeScreens.forEach(s => {
        // Cost = BasePrice * Dayparting Premium * Hours * Days * SOV
        totalCost += (s.basePrice * strategy.multiplier * strategy.hours * days * sovMultiplier);
        // Impressions = Footfall * (Hours/15) * Days * SOV (簡單按比例計人流)
  ...
  // Step 1: Campaign Setup (大客一開波係定策略)
  const renderStep1 = () => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 h-[350px]">
        <div className="md:col-span-2 bg-slate-200 rounded-xl border border-slate-300 flex items-center justify-center relative overflow-hidden">
            ...
            <div className="text-center relative z-10">
                ...
                <p className="font-bold text-slate-600">地圖預覽區 (Interactive Map)</p>
            </div>
        </div>
        
        <div className="bg-white border border-slate-200 rounded-xl flex flex-col overflow-hidden shadow-sm">
            <div className="p-3 bg-slate-50 border-b font-bold text-sm">機位名單 ({activeScreens.length})</div>
                ...
                {MOCK_SCREENS.map(screen => {
                    ...
                    <div key={screen.id} onClick={() => !isBanned && handleToggleScreen(screen.id)} className={`p-3 rounded-lg border flex justify-between items-center transition-all ${isBanned ? 'bg-slate-50 border-slate-100 opacity-50 cursor-not-allowed' : selectedScreens.has(screen.id) ? 'border-blue-500 bg-blue-50 cursor-pointer' : 'border-slate-200 bg-white cursor-pointer hover:border-blue-300'}`}>
                        <div>
                            ...
                            <p className="text-[10px] text-slate-500 flex items-center gap-1 mt-1"><Users size={10}/> 預估日人流: {(screen.footfall/1000).toFixed(0)}k</p>
                        </div>
                        {!isBanned && (
                            <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${selectedScreens.has(screen.id) ? 'border-blue-600 bg-blue-600 text-white' : 'border-slate-300'}`}>
      ...
      <div className="space-y-6 animate-in fade-in flex flex-col items-center">
        <div className="w-full max-w-2xl bg-white border border-slate-200 shadow-sm rounded-2xl p-8 relative overflow-hidden">
            ...
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 mb-6 space-y-3 text-sm">
                ...
                <div className="flex justify-between"><span className="text-slate-500">覆蓋</span><span className="font-bold">{activeScreens.length} 部屏幕</span></div>
          ...
          <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
              <div className="bg-white p-10 rounded-3xl shadow-xl text-center max-w-lg w-full">
                  ...
                  <p className="text-slate-600 mb-6 text-sm">
                      我哋嘅 Account Manager 會喺 1 個工作天內聯絡你，並將正式嘅 Invoice 同 Media Plan PDF Send 去你 Email。
                  </p>
                  <div className="flex gap-3">
                      <button className="flex-1 border border-slate-300 text-slate-700 py-3 rounded-xl font-bold text-sm hover:bg-slate-50 flex justify-center items-center gap-2">
                          <Download size={16}/> 下載 PDF 備份
                      </button>
                      <button onClick={() => window.location.reload()} className="flex-1 bg-slate-900 text-white py-3 rounded-xl font-bold text-sm hover:bg-slate-800">
                          返回主頁
                      </button>
                  </div>
              </div>
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-sm p-4 mb-6 flex justify-between items-center border border-slate-200">
            <div className="flex items-center gap-3">
                ...
                <div>
                    <h1 className="font-black text-lg text-slate-900 leading-tight">DOOH Enterprise</h1>
                    <p className="text-[10px] text-slate-500 uppercase tracking-widest font-bold">Media Planner Portal</p>
                </div>
            </div>
        ...
        <div className="flex justify-between items-center mt-6">
            <button 
                ...
                className={`px-6 py-3 rounded-xl font-bold flex items-center gap-2 transition-all ${step === 1 ? 'opacity-0 pointer-events-none' : 'bg-white border border-slate-300 text-slate-600 hover:bg-slate-50'}`}
            >
            ...
            {step < 4 ? (
                <button 
                    ...
                    className="px-8 py-3 rounded-xl font-bold flex items-center gap-2 bg-blue-600 text-white hover:bg-blue-700 shadow-lg shadow-blue-200 transition-all active:scale-95"
                >
                    Next Step <ChevronRight size={18}/>
                </button>
            ) : (
                <div className="flex gap-3">
                    <button className="px-6 py-3 rounded-xl font-bold flex items-center gap-2 border-2 border-slate-200 text-slate-600 hover:bg-slate-50 transition-all">
                        <Download size={18}/> Export Media Plan
                    </button>
                    <button 
                        onClick={() => setIsSubmitted(true)}
                        className="px-8 py-3 rounded-xl font-bold flex items-center gap-2 bg-slate-900 text-white hover:bg-slate-800 shadow-xl transition-all active:scale-95"
                    >
